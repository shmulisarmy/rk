...
function CartItem(key, amount){
    ...
    assert(typeof amount == "number", "amount must be a number")
        ...
        <div class="item">
            ...
            <h2>${name}</h2>
            <p>$${price}</p>
            <button onclick=${(event) => { localStorageCartPlus(key); rerender("cart")}} style="height: 20px; width: 20px;">+</button>
            <p>${amount}</p>
            <button onclick=${(event) =>{ localStorageCartMinus(key); rerender("cart")}} style="height: 20px; width: 20px;">-</button>
        </div>
    `
}

function Cart(){
    const updatePrice = createSignal("$" + calculate_price(), "cart-total")
    ...
    <div class="container">
        ...
        /${Object.keys(localStorageCart).map(key => CartItem(key, localStorageCart[key])).join("")}
    </div>
    <h3 signal="cart-total" style="text-align: center; font-size: 50px; color: #111; font-family: 'Courier New', Courier, monospace; text-shadow: 1px 1px 2px #ccc;">Total: $${calculate_price()}</h3>
    </div>`
}
...
root.outerHTML = App({showing: "menu"})
